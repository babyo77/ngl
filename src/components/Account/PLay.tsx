...
const getDetails = async ()=>{
 ...
 const res = await axios.post(`${apiUrl}/api/user/details`,token)
 return res.data
}

const {data:loggedData,isLoading,isError} = useQuery<user>("details",getDetails,{
...
const handleShare = async ()=>{
    ...
    const handleCopy = ()=>{
        if(linkRef.current?.textContent){
            navigator.clipboard.writeText((linkRef.current.textContent).toLowerCase()).then(()=>{
                alert("🔗 link copied")
            })
        }
    }
   ...
   <h1 className="text-white font-bold text-2xl w-[70vw] text-center">send me anonymous messages!</h1>
      </div>
   ...
   <p  ref={linkRef} className="text-zinc-500  w-[70vw]  overflow-auto font-semibold text-[.7rem]  bg-transparent uppercase text-center">
